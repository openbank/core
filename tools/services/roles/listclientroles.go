...
func (s *ClientsServer) ListClientRoles(ctx context.Context, req *pb.ListClientRolesRequest) (*pb.ListClientRolesResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.clients.roles.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListRoleClientRoles(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
	results := make([]*pb.ClientRole, 0, len(v))
	for _, w := range v {
		pb, err := w.PB()
		if err != nil {
			return nil, err
	...
	return &pb.ListClientRolesResponse{
		Total:     stat.Total,
