...
func (s *DepositServer) ListDepositTransactions(ctx context.Context, req *pb.ListDepositTransactionsRequest) (*pb.ListDepositTransactionsResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.deposits.transactions.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListTransactionDepositTransactions(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
	results := make([]*pb.DepositTransaction, 0, len(v))
	for _, w := range v {
		pb, err := w.PB()
		if err != nil {
			return nil, err
	...
	return &pb.ListDepositTransactionsResponse{
		Total:        stat.Total,
