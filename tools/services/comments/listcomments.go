...
func (s *Server) ListComments(ctx context.Context, req *pb.ListCommentsRequest) (*pb.ListCommentsResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.comments.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListComments(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
	results := make([]*pb.Comment, 0, len(v))
	for _, w := range v {
		pb, err := w.PB()
		if err != nil {
			return nil, err
