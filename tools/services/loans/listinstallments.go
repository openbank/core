...
func (s *Server) ListInstallments(ctx context.Context, req *pb.ListInstallmentsRequest) (*pb.ListInstallmentsResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.accounts.loans.installments.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListLoanInstallments(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
	results := make([]*pb.Installment, 0, len(v))
	for _, w := range v {
		pb, err := w.PB()
		if err != nil {
			return nil, err
	...
	return &pb.ListInstallmentsResponse{
		Total:        stat.Total,
