...
func (s *Server) ListDocuments(ctx context.Context, req *pb.ListDocumentsRequest) (*pb.ListDocumentsResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.docs.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListDocuments(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
	results := make([]*pb.Document, 0, len(v))
	for _, w := range v {
		pb, err := w.PB()
		if err != nil {
			return nil, err
