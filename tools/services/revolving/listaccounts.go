...
func (s *AccountsServer) ListAccounts(ctx context.Context, req *pb.ListAccountsRequest) (*pb.ListAccountsResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.revolving.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListRevolvingAccounts(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
