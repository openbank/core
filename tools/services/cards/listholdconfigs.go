...
func (s *HoldConfigServer) ListHoldConfigs(ctx context.Context, req *pb.ListHoldConfigsRequest) (*pb.ListHoldConfigsResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.holds.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListCardHoldConfigs(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
	results := make([]*pb.HoldConfig, 0, len(v))
	for _, w := range v {
		pb, err := w.PB()
		if err != nil {
			return nil, err
	...
	return &pb.ListHoldConfigsResponse{
		Total:     stat.Total,
