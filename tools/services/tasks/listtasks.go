...
func (s *Server) ListTasks(ctx context.Context, req *pb.ListTasksRequest) (*pb.ListTasksResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.tasks.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListTasks(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
	results := make([]*pb.Task, 0, len(v))
	for _, w := range v {
		pb, err := w.PB()
		if err != nil {
			return nil, err
