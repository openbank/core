...
func (s *RatesServer) ListAccountingRates(ctx context.Context, req *pb.ListAccountingRatesRequest) (*pb.ListAccountingRatesResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.currencies.accounting_rate.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListCurrencyAccountingRates(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
	results := make([]*pb.AccountingRate, 0, len(v))
	for _, w := range v {
		pb, err := w.PB()
		if err != nil {
			return nil, err
	...
	return &pb.ListAccountingRatesResponse{
		Total:     stat.Total,
