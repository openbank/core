...
func (s *Server) ListCurrencies(ctx context.Context, req *pb.ListCurrenciesRequest) (*pb.ListCurrenciesResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.currencies.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListCurrencies(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
	results := make([]*pb.Currency, 0, len(v))
	for _, w := range v {
		pb, err := w.PB()
		if err != nil {
			return nil, err
