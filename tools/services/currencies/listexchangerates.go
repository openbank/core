...
func (s *RatesServer) ListExchangeRates(ctx context.Context, req *pb.ListExchangeRatesRequest) (*pb.ListExchangeRatesResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.currencies.exchange_rate.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListCurrencyExchangeRates(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
	results := make([]*pb.ExchangeRate, 0, len(v))
	for _, w := range v {
		pb, err := w.PB()
		if err != nil {
			return nil, err
