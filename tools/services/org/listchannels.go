...
func (s *ChannelsServer) ListChannels(ctx context.Context, req *pb.ListChannelsRequest) (*pb.ListChannelsResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.organization.channels.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListOrgChannels(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
	results := make([]*pb.Channel, 0, len(v))
	for _, w := range v {
		pb, err := w.PB()
		if err != nil {
			return nil, err
