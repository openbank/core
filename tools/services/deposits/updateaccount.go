...
func (s *Server) UpdateAccount(ctx context.Context, req *pb.UpdateAccountRequest) (*pb.Account, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.accounts.deposits.update"); err != nil {
		return nil, err
	}
	...
	v, err := db.NewDepositAccount(req.Body)
	if err != nil {
		return nil, err
	}
	if err := storage.UpdateDepositAccountByAccountID(ctx, &v); err != nil {
		return nil, err
	}
	return req.Body, nil
}
