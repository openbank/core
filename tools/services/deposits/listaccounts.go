...
func (s *Server) ListAccounts(ctx context.Context, req *pb.ListAccountsRequest) (*pb.ListAccountsResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.accounts.deposits.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListDepositAccounts(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
