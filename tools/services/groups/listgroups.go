...
func (s *Server) ListGroups(ctx context.Context, req *pb.ListGroupsRequest) (*pb.ListGroupsResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.groups.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListGroups(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
	results := make([]*pb.Group, 0, len(v))
	for _, w := range v {
		pb, err := w.PB()
		if err != nil {
			return nil, err
