...
func (s *Server) ListClients(ctx context.Context, req *pb.ListClientsRequest) (*pb.ListClientsResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.clients.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListClients(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
	results := make([]*pb.Client, 0, len(v))
	for _, w := range v {
		pb, err := w.PB()
		if err != nil {
			return nil, err
