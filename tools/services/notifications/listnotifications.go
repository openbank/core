...
func (s *Server) ListNotifications(ctx context.Context, req *pb.ListNotificationsRequest) (*pb.ListNotificationsResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.notifications.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListNotifications(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
	results := make([]*pb.Notification, 0, len(v))
	for _, w := range v {
		pb, err := w.PB()
		if err != nil {
			return nil, err
