...
func (s *Server) ListBranches(ctx context.Context, req *pb.ListBranchesRequest) (*pb.ListBranchesResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.branches.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListBranches(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
	results := make([]*pb.Branch, 0, len(v))
	for _, w := range v {
		pb, err := w.PB()
		if err != nil {
			return nil, err
