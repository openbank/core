...
func (s *Server) ListSets(ctx context.Context, req *pb.ListSetsRequest) (*pb.ListSetsResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.fields.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListFieldSets(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
	results := make([]*pb.Set, 0, len(v))
	for _, w := range v {
		pb, err := w.PB()
		if err != nil {
			return nil, err
