...
func (s *Server) ListSources(ctx context.Context, req *pb.ListSourcesRequest) (*pb.ListSourcesResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.sources.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListRateSources(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
	results := make([]*pb.Source, 0, len(v))
	for _, w := range v {
		pb, err := w.PB()
		if err != nil {
			return nil, err
