...
func (s *Server) ListIndexRates(ctx context.Context, req *pb.ListIndexRatesRequest) (*pb.ListIndexRatesResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.sources.indexrates.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListRateIndexRates(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
	results := make([]*pb.IndexRate, 0, len(v))
	for _, w := range v {
		pb, err := w.PB()
		if err != nil {
			return nil, err
	...
	return &pb.ListIndexRatesResponse{
		Total:     stat.Total,
