...
func (s *EntriesServer) ListEntries(ctx context.Context, req *pb.ListEntriesRequest) (*pb.ListEntriesResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.ledgers.entries.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListLedgerEntries(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
	results := make([]*pb.Entry, 0, len(v))
	for _, w := range v {
		pb, err := w.PB()
		if err != nil {
			return nil, err
