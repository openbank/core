...
func (s *Server) ListCentres(ctx context.Context, req *pb.ListCentresRequest) (*pb.ListCentresResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.centres.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListCentres(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
	results := make([]*pb.Centre, 0, len(v))
	for _, w := range v {
		pb, err := w.PB()
		if err != nil {
			return nil, err
