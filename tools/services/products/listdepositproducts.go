...
func (s *DepositsServer) ListDepositProducts(ctx context.Context, req *pb.ListDepositProductsRequest) (*pb.ListDepositProductsResponse, error) {
	if err := s.Auth.CheckPerm(ctx, req, "v1.products.deposit.list"); err != nil {
		return nil, err
	}
	...
	stat, v, newAfter, err := storage.ListProductDepositProducts(ctx, req.Filter, req.PageSize, req.OrderBy, after)
	if err != nil {
		return nil, err
	}
	results := make([]*pb.DepositProduct, 0, len(v))
	for _, w := range v {
		pb, err := w.PB()
		if err != nil {
			return nil, err
	...
	return &pb.ListDepositProductsResponse{
		Total:     stat.Total,
